...
def _MakeSpeechResponse(canonical_prayer, desired_prayer, prayer_time, prayer_time_prop,
    ...
    if prayer_time and locality:
        ...
        location = locality[1] if locality[0] == Locality.CITY else GetMasjidDisplayName(
            locality[1])
        ...
        if prayer_time_prop and prayer_time_prop.lower() == 'time until':
            ...
            if ((prayer_time[1] - canonical_prayer == 1) or (prayer_time[1] - canonical_prayer == -5)
                ...
                if desired_prayer.lower() == 'suhur':
                    ...
                    pronunciation_next_prayer = util.GetPronunciation(
                        next_prayer)
...
class IntentHandler(object):
    ...
    def HandleIntent(self, post_params):
        ...
        if not has_location:
            ...
            # this should be filled on PERMISSION_REQUEST intents in the
            # relevant context
            permission_context = _GetContext(post_params, 'requ')
            #print 'permission context = ', permission_context
            desired_prayer = permission_context.get(
                'parameters').get('PrayerName')
            prayer_time_prop = permission_context.get(
                'parameters').get('prayer-time-prop')

        # this should also always be available
        user_id = post_params.get('originalRequest').get('data').get('user').get(
            'userId')

        # if we have a masjid name then call GetIqamaTime to obtain it from
        # scraper
        if masjid:
        ...
        if city:
            return self._RespondToCityRequest(
                params, desired_prayer, prayer_time_prop)
    ...
    def _LookupOrRequestInformation(self, post_params, params, desired_prayer,
        ...
        permission_context = _GetContext(
            post_params, 'actions_intent_permission')
        if (permission_context and
                permission_context.get('parameters').get('PERMISSION') == 'false'):
            # if we are here it means the user rejected our request to use
            # location
            return {
                'speech':
        ...
        if (not explicit_location_requested and user_info and
            ...
            return self._ComputePrayerTimeAndRespond(
                desired_prayer, lat, lng, city, prayer_time_prop)
    ...
    def _ComputePrayerTimeProperty(
            self, canonical_prayer, all_prayer_times, lat, lng):
        ...
        if current_user_time:
            ...
            for prayer in all_prayer_times:
                # skip prayer if it is [0] unspecified, [7] qiyam, or [2]
                # sunrise
                ...
                if not prayer_time:
                    return None
                # get the time difference between the current user time and the
                # loop prayer time
                time_until_params = util.GetTimeDifference(
                    current_user_time, prayer_time)
            ...
            if canonical_prayer and not canonical_prayer == 'NA':
                ...
                desired_prayer_time_diff = util.GetTimeDifference(
                    current_user_time, desired_prayer_time)
            return (desired_prayer_time_diff, next_prayer,
                    next_prayer_time, next_prayer_time_diff)
        else:
            return None

    def _ComputePrayerTimeAndRespond(
            self, desired_prayer, lat, lng, city, prayer_time_prop):
        if not city:
            city = 'your location'
